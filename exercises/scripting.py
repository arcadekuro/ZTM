...
"""
A module in python is simply a file that contains accesible code:
with functions and statements.
...
"""
Sys
provides info about constants, functions & methods of the Python intepreter
    - it is the system specific parameters and functions.
    Responsible for controlling and interacting with the interpreter.
    (e.g. info on operating system or python version. )
    .stdin
    .stdout
...
print(sys.argv)  # list containing the CLI arguments passed to our scripts
# (script name & path)
...
def changes_os():
    ...
    # os.chdir() #change directory & give path. then you can print the cwd.
    # only double backslash or 1 front slask works
...
lets you interact with os to create new processes to pass the information
into and out of them and get the return codes
so it ca be used by using the coil function.
